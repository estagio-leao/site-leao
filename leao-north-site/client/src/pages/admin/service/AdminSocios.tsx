/*
 * LEÃO NORTH — Painel Admin: Sócios — Fase 23 / 25
 * CRUD da entidade socios via api/service/socios.php (GET) e
 * api/admin/service/*_socio.php (POST/DELETE) com foto ÚNICA opcional.
 *
 * Fase 25 (UX): listagem full-width + botão "Novo Sócio" no topo + formulário
 * em Modal (AdminDialog, size lg). O fechamento revoga ObjectURL do preview.
 */
  ...
  return (
    <div className="w-full">
      ...
      <AdminDialog
        ...
        title={editandoId != null ? "Editar Sócio" : "Novo Sócio"}
        description="Cadastro de sócio exibido no site"
        size="lg"
        footer={
          <>
            <button type="submit" form="admin-socio-form" disabled={loading} className={goldButtonClass}>
      ...
      >
        <form id="admin-socio-form" onSubmit={handleSave} className="space-y-4">
          ...
          <div>
            <label className="block text-white/40 text-xs tracking-widest uppercase mb-1.5">
              Foto ({editandoId != null ? "opcional — substituir ou remover" : "opcional"})
            </label>
            ...
            {fotoPreview ? (
              <div className="relative mt-3">
                <img src={fotoPreview} alt="Foto do sócio" className="w-full h-44 object-cover rounded-sm border border-white/10" />
                {editandoId != null && !fotoFile && (
                  <button
                    type="button"
