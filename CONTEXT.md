...
| [`Service.tsx`](leao-north-site/client/src/pages/Service.tsx) | **Leão North Service:** compõe `Navbar` → Hero → About → Mission → **Services (dinâmico)** → **Sócios (dinâmico)** → **Portfolio (dinâmico, cards com mini-carrossel)** → Differentials → Testimonials → Contact → Footer → WhatsAppButton. Fundo `#080808`. **Fase 37:** **Sócios passou a vir ANTES do Portfólio** (pedido do cliente), com o menu do [`Navbar.tsx`](leao-north-site/client/src/components/Navbar.tsx) e os Links Rápidos do [`Footer.tsx`](leao-north-site/client/src/components/Footer.tsx) na mesma ordem (Sócios antes de Portfólio). ⚠️ As âncoras **não mudaram** (`#socios` e `#portfolio` continuam nos mesmos `id`s), portanto nenhum link interno quebra — inclusive nas subpáginas, que seguem usando [`lib/anchorScroll.ts`](leao-north-site/client/src/lib/anchorScroll.ts) |
...
| Arquivo | Conteúdo |
| --- | --- |
| [`HeroSection.tsx`](leao-north-site/client/src/components/sections/HeroSection.tsx) | Hero assimétrico: badge, título, CTAs (WhatsApp + scroll), estatísticas, imagem com clip-path diagonal. **Fase 37:** o stat **"Avaliação Google"** deixou de ser fixo (`4.2★`) e passou a exibir a **média real** dos depoimentos (`mediaGlobal`/`totalGlobal` de [`api/depoimentos.php`](api/depoimentos.php) via [`lib/depoimentos.ts`](leao-north-site/client/src/lib/depoimentos.ts)), no **mesmo formato pt-BR** da seção de depoimentos (`formatarMedia` → "4,8★") — alterar as estrelas no painel muda os dois lugares; enquanto a API carrega, mantém o `4.2★` para não "piscar" |
